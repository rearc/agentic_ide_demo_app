...
def migration_db(tmp_path):
    ...
    config = AlembicConfig(f'{MIGRATIONS_DIR}/alembic.ini')
...
def test_cards_table_has_the_columns_the_model_expects(migration_db):
    config, engine = migration_db

    command.upgrade(config, 'head')

    columns = {c['name'] for c in sa.inspect(engine).get_columns('cards')}
    assert {
...
def test_todos_table_has_the_columns_the_model_expects(migration_db):
    config, engine = migration_db

    command.upgrade(config, 'head')

    columns = {c['name'] for c in sa.inspect(engine).get_columns('todos')}
    assert {'id', 'card_id', 'text', 'done', 'created_at', 'updated_at'} <= columns
